...
class ParallelModel(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 64, (3, 5), (1, 1), (1, 2)),
            nn.BatchNorm2d(64),
            nn.LeakyReLU(),
            nn.MaxPool2d((4, 2)),
            nn.Dropout(),

            nn.Conv2d(64, 128, (3, 5), (1, 1), (1, 2)),
            nn.BatchNorm2d(128),
            nn.LeakyReLU(),
            nn.Dropout(),

            nn.Conv2d(128, 256, (3, 5), (1, 1), (1, 2)),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(),
            nn.Dropout(),

            nn.Conv2d(256, 256, (3, 5), (1, 1), (1, 2)),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(),
            nn.Dropout(),
    ...
    def forward(self, x):
        feature = self.conv(x)
        feature1 = self.fc1(feature.view(-1, 2560))
        r_out, (h_n, h_c) = self.lstm(feature1.view(-1, 160, 768), None)
        output = self.fc2(r_out[:, -1, :])
...
class ParallelModel1(nn.Module):
    def __init__(self, num_emotions):
        ...
        self.conv2Dblock = nn.Sequential(
            ...
            nn.Conv2d(in_channels=1, out_channels=16, kernel_size=(3, 5), stride=1, padding=(1, 2)),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Dropout(p=0.5),

            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(3, 5), stride=1, padding=(1, 2)),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            ...
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 5), stride=1, padding=(1, 2)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        ...
        self.transf_maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        transf_layer = nn.TransformerEncoderLayer(d_model=60, nhead=4, dim_feedforward=512, dropout=0.5,
                                                  activation='relu')
        self.transf_encoder = nn.TransformerEncoder(transf_layer, num_layers=2)

        self.out_linear = nn.Linear(316, num_emotions)
    ...
    def forward(self, x):
        ...
        x_reduced = x_reduced.view(-1, 60, 150)
